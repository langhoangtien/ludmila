...
export default function ProductDetailsCarousel({ product }: Props) {
  ...
  const imagesLightbox = product.photos.map((img) => ({ src: img.path }));
  ...
  const carouselSettings2 = {
    ...
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: product.photos.length > 3 ? 3 : product.photos.length,
  };
      ...
      <Carousel {...carouselSettings1} asNavFor={nav2} ref={carousel1}>
        {product.photos.map((img) => (
          <Image
            ...
            src={img.path}
            ratio="1/1"
            onClick={() => handleOpenLightbox(img.path)}
            sx={{ cursor: 'zoom-in' }}
          />
        ))}
      </Carousel>
  ...
  const renderThumbnails = (
    <StyledThumbnailsContainer length={product.photos.length}>
