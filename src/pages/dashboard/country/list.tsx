...
import CountryTableToolbar from '@/sections/@dashboard/country/list/CountryTableToolbar';
import CountryTableRow from '@/sections/@dashboard/country/list/CountryTableRow';
...
BrandListPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;
...
export default function BrandListPage() {
  ...
  const getData = async () => {
    ...
    try {
      const respon = await axios.get(`/api/v1/countries?${params}`);
  ...
  const handleDeleteRows = async (selectedRows: string[]) => {
    try {
      await axios.delete(`/api/v1/countries/delete`, { data: selectedRows });
      enqueueSnackbar(translate('notification.delete.success'));
      setSelected([]);
      setRefresh(!refesh);
    } catch (error) {
